...
impl Report {
    ...
    pub fn queries(&self, points: &[Query]) -> Result<()> {
    ...
    pub fn error_header(&self, f: &mut impl Write) -> Result<()> {
        writeln!(f,"polarization,cells,nodes,estimated_scalar_bytes,component,scale,max_abs,rms_abs,max_normalized,rms_normalized,refinement_ratio,max_rate,rms_rate,provisional_target,target_met")?;
        Ok(())
    }
    pub fn errors(
        &self,
        ...
        for i in 0..9 {
            ...
            println!(
                "{} n={} {:>9}: max={:.3e} rms={:.3e} p(max)={} target={}",
                polarization_name(g.pol),
                cells,
                COMPONENTS[i],
                max,
